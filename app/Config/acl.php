 ...
 * Assumptions:
 *
 * 1. In your application you created a Usuario model with the following properties:
 ...
 *
 *    array('Usuario' => array('username' => 'jeff', 'group_id' => 4, ...))
 *
 * as ARO and
 *
 ...
 *       'Role' => 'Usuario/group_id',
 *    );
 *
 * then PhpAcl will lookup if we defined a role like Usuario/jeff. If that role is not found, PhpAcl will try to
 ...
 *       ),
 *       'deny' => array(
 *       	'controllers/invoices/delete' => 'Role/accountant, Usuario/jeff',
 *       	'controllers/articles/(delete|publish)' => 'Role/editor',
 *       ),
 *    );
 *
 * Ok, so as jeff inherits from Role/manager he's matched every rule that references Usuario/jeff, Role/manager,
 * Role/editor, Role/accountant and Role/default. However, for jeff, rules for Usuario/jeff are more specific than
 ...
 * rule denies access to the delete action explicitly for Usuario/jeff, so he'll be denied access to the resource.
 *
 * If we would remove the role definition for Usuario/jeff, then jeff would be granted access as he would be resolved
