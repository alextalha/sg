	...
	<div class="um_box_mid">
		<div class="um_box_mid_content">
			<div class="um_box_mid_content_top">
				<div class="umstyle1"  align="left" style="float:left"><?php echo __('Usuario Group Permissions');?></div>
				<div style="float:right">
					<span  class="umstyle2"><?php __('Select Controller');?></span>  <?php echo $this->Form->input("controller",array('type'=>'select','div'=>false,'options'=>$allControllers,'selected'=>$c,'label'=>false,"onchange"=>"window.location='".SITE_URL."permissoes/?c='+(this).value"))?>
				</div>
						...
						if (!empty($value)) {
							for ($i=0; $i<count($value); $i++) {
								if (isset($value[$i])) {
									...
									echo "<td>".$action."</td>";
									echo "<td>";
									foreach ($grupos as $user_group) {
										$ugname=$user_group['nome'];
										...
										echo $this->Form->input($ugname,array('label'=>$ugname, 'id'=>$ugname_alias.$k,'type'=>'checkbox','checked'=>$checked));
									}
									echo "</td>";
									echo "<td>";
